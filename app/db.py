...
-- ECCEZIONE consapevole al principio privacy-first dei Fascicoli: il CONTENUTO
-- del foglio vive ONLINE (Postgres) perche' piu' utenti lo modificano insieme
-- da macchine diverse. `project_id` e' NULLABLE: un foglio puo' essere
-- standalone (asset del tenant, accessibile da qualsiasi postazione) oppure
-- agganciato a un fascicolo. Redis (opzionale) e' solo bus realtime, MAI fonte
-- di verita': il recupero dopo reconnect avviene da project_sheet_revisions.
-- Vedi docs/argos_fogli_collaborativi_plan.md.
-- ============================================================
...
CREATE INDEX IF NOT EXISTS idx_project_sheets_tenant   ON project_sheets(tenant_id, is_archived);
CREATE INDEX IF NOT EXISTS idx_project_sheets_project  ON project_sheets(project_id, is_archived);
...
CREATE INDEX IF NOT EXISTS idx_project_sheet_cells_sheet_revision ON project_sheet_cells(sheet_id, revision);
