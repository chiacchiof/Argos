@echo off
REM Argos - prima installazione (wrapper per scripts\install_client.ps1).
REM Doppio-click su questo file per installare, oppure: .\install.bat da PowerShell.
REM
...
if not exist "scripts\install_client.ps1" (
    echo.
    echo [ERROR] scripts\install_client.ps1 non trovato.
...
powershell.exe -NoProfile -ExecutionPolicy Bypass -File ".\scripts\install_client.ps1"
set "EXITCODE=%ERRORLEVEL%"

echo.
echo ============================================================
if "%EXITCODE%"=="0" (
    echo Installazione completata. Premi un tasto per chiudere la finestra.
) else (
    echo Installazione terminata con errori ^(exit %EXITCODE%^). Leggi i messaggi qui sopra.
)
echo ============================================================
pause >nul
exit /b %EXITCODE%
